...
@[code lang=java transcludeWith=:::block](@/reference/1.21.1/src/main/java/com/example/docs/appearance/ExampleModAppearance.java)
...
Even if your block's texture is transparent or translucent, it will still appear opaque. To fix this, you need to set your block's _Render Type_.

Render Types are categories used to group different types of block surfaces for rendering. This allows the game to use the correct visual effects and optimizations for each type.

We need to register our block with the correct Render Type. On 1.21.1, the most common of the options vanilla provides are:

- `solid()`: The default, a solid block without any transparency.
- `cutout()` and `cutoutMipped()`: A block that makes use of transparency, for example Glass or Flowers. `cutoutMipped()` will look better at a distance.
...
In your **client initializer**, register your block with the correct `RenderType` using Fabric API's `BlockRenderLayerMap`.

@[code lang=java transcludeWith=:::block_render_layer_map](@/reference/1.21.1/src/client/java/com/example/docs/appearance/ExampleModAppearanceClient.java)
...
@[code lang=java transcludeWith=:::color_provider](@/reference/1.21.1/src/client/java/com/example/docs/appearance/ExampleModAppearanceClient.java)
