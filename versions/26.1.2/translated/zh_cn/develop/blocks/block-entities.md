...
@[code transcludeWith=:::1](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

注册一个 `BlockEntity` 会得到一个 `BlockEntityType`，就像我们上面使用的 `COUNTER_BLOCK_ENTITY`：

@[code transcludeWith=:::1](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/ModBlockEntities.java)
...
@[code transcludeWith=:::1](@/reference/26.1.2/src/main/java/com/example/docs/block/custom/CounterBlock.java)
...
@[code transcludeWith=:::5](@/reference/26.1.2/src/main/java/com/example/docs/block/ModBlocks.java)

## 使用方块实体 {#using-the-block-entity}

现在我们有了一个方块实体，我们可以用它来存储方块被右键点击的次数。 我们将通过向 `CounterBlockEntity` 类添加一个 `clicks` 字段来实现：

@[code transcludeWith=:::2](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
@[code transcludeWith=:::2](@/reference/26.1.2/src/main/java/com/example/docs/block/custom/CounterBlock.java)
...
@[code transcludeWith=:::3](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

在这里，我们将应该保存的字段添加到传入的 `ValueOutput` 中：对于计数器方块，就是 `clicks` 字段。

读取是类似的，你从 `ValueInput` 中获取之前保存的值，并将它们保存在 BlockEntity 的字段中：

@[code transcludeWith=:::4](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
@[code transcludeWith=:::7](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

现在，当玩家登录或移动到一个存在该方块的区块时，他们将立即看到正确的计数器值。

## 同步数据 {#syncing-data}

当新玩家载入方块时，他们会看到正确的计数，但其他正在旁观交互的玩家却看不到计数更新。 这种现象称为不同步，当服务器更新了状态而客户端尚未更新时就会发生这种情况。

为了解决这个问题，我们可以使用方块实体更新包。 重写 `getUpdatePacket` 方法，并返回一个包含方块数据的包，该数据来自我们的 `getUpdateTag` 方法。

<<< @/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java#update-packet

然后，重写 `setChanged` 方法，以便在方块实体发生变化时广播数据。

<<< @/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java#broadcast-update
...
@[code transcludeWith=:::3](@/reference/26.1.2/src/main/java/com/example/docs/block/custom/CounterBlock.java)
...
@[code transcludeWith=:::5](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

别忘了序列化和反序列化这个字段！

现在我们可以使用 `ticksSinceLast` 来检查在 `incrementClicks` 中是否可以增加计数器：

@[code transcludeWith=:::6](@/reference/26.1.2/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
