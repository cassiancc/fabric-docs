...
authors:
  ...
  - ekulxam
resources:
  https://minecraft.wiki/w/Data_component_format#List_of_components: 原版组件列表 - Minecraft Wiki
---
...
@[code transcludeWith=::1](@/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java)
...
@[code transcludeWith=::2](@/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java)
...
@[code transcludeWith=::1](@/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java)
...
这会返回组件的当前值，其类型为我们注册组件时定义的类型。 可以将这个值添加到物品提示中。

### 添加工具提示 {#append-tooltips}

为了简单起见，我们将这行添加到 `CounterItem` 类的 `appendHoverText` 方法中。 请注意，此方法已被弃用，因为 Mojang 致力于确保物品行为完全通过组件而不是通过物品来处理。
...
@[code transcludeWith=::1](@/reference/26.1.2/src/main/java/com/example/docs/component/ComponentWithTooltip.java)

然后，你 可以通过 `ItemComponentTooltipProviderRegistry` 注册 `TooltipProvider`。 这会在`ModInitializer`中的`onInitalize`方法里被调用。

@[code lang=java transcludeWith=#tooltip_provider](@/reference/26.1.2/src/main/java/com/example/docs/ExampleMod.java)

或者你可以使用`ItemTooltipCallback`来取代`appendHoverText`。 这会在`ClientModInitializer`的`onInitializeClient`方法中被调用。

@[code lang=java transcludeWith=#tooltip_provider_client](@/reference/26.1.2/src/client/java/com/example/docs/ExampleModClient.java)
...
@[code transcludeWith=::\_13](@/reference/26.1.2/src/main/java/com/example/docs/item/ModItems.java)
...
@[code transcludeWith=::3](@/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java)
...
@[code transcludeWith=::use](@/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java)
...
@[code transcludeWith=::codec](@/reference/26.1.2/src/main/java/com/example/docs/component/AdvancedCustomComponent.java)
...
@[code transcludeWith=::3](@/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java)
...
### 添加高级工具提示 {#advanced-tooltips}

由于我们有自定义组件类，因此可以按此方式实现 `TooltipProvider`，从而避免使用已弃用的 `appendHoverText` 方法。

@[code transcludeWith=::1](@/reference/26.1.2/src/main/java/com/example/docs/component/AdvancedCustomComponent.java)

然后，你 可以通过 `ItemComponentTooltipProviderRegistry` 注册 `TooltipProvider`。 这会在`ModInitializer`中的`onInitalize`方法里被调用。

@[code lang=java transcludeWith=#advanced_tooltip_provider](@/reference/26.1.2/src/main/java/com/example/docs/ExampleMod.java)
