...
description: Lerne, wie man die Klasse GuiGraphicsExtractor verwendet, um verschiedene Formen, Texte und Texturen zu rendern.
authors:
  - IMB11
---
...
Die Klasse `GuiGraphicsExtractor` ist die Hauptklasse, die für das Rendering im Spiel verwendet wird. Sie wird für das Rendern von Formen, Text und Texturen verwendet und, wie zuvor gesehen, für die Bearbeitung von `PoseStack`s und die Verwendung von `BufferBuilder`n.

## Formen zeichnen {#drawing-shapes}

Die Klasse `GuiGraphicsExtractor` kann verwendet werden, um auf einfache Weise **quadratische** Formen zu zeichnen. Wenn du Dreiecke oder andere nicht-quadratische Formen zeichnen willst, musst du einen `BufferBuilder` verwenden.
...
@[code lang=java transcludeWith=:::1](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)
...
@[code lang=java transcludeWith=:::2](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)
...
@[code lang=java transcludeWith=:::3](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)

![Linien](/assets/develop/rendering/draw-context-lines.png)

## Der Scheren-Manager {#the-scissor-manager}

Die Klasse `GuiGraphicsExtractor` hat einen eingebauten Scheren-Manager. So kannst du dein Rendering ganz einfach auf einen bestimmten Bereich beschränken. Dies ist nützlich, um Dinge wie Tooltips oder anderen Elemente, die nicht außerhalb eines bestimmten Bereichs gerendert werden sollen, zu rendern.

### Den Scheren-Manager verwenden {#using-the-scissor-manager}
...
@[code lang=java transcludeWith=:::4](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)
...
Im Allgemeinen wird empfohlen, dass man die Überladung verwendet, die die Parameter `textureWidth` und `textureHeight` angibt. Der Grund dafür ist, dass die Klasse `GuiGraphicsExtractor` diese Werte entgegennimmt, wenn du sie nicht angibst, was manchmal falsch sein kann.
...
@[code lang=java transcludeWith=:::5](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)
...
@[code lang=java transcludeWith=:::6](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)

![Textur der Region](/assets/develop/rendering/draw-context-region-texture.png)

## Zeichnen von Text {#drawing-text}

Die Klasse `GuiGraphicsExtractor` verfügt über verschiedene selbsterklärende Methoden zum Rendern von Texten, die hier aus Gründen der Kürze nicht behandelt werden.

Nehmen wir an, wir wollen "Hello World" auf die Oberfläche zeichnen. Wir können dazu die Methode `GuiGraphicsExtractor.text(...)` verwenden.
...
@[code lang=java transcludeWith=:::7](@/reference/26.1.2/src/client/java/com/example/docs/rendering/DrawContextExampleScreen.java)
