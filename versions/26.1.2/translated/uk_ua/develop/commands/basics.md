...
[Brigadier](https://github.com/Mojang/brigadier) — це синтаксичний аналізатор і диспетчер команд із відкритим кодом, створений Mojang для Minecraft. Це деревоподібна бібліотека команд, де ви створюєте дерево команд і аргументів.
...
Ціле число можна вважати результатом виконання команди. Зазвичай значення, що менше або дорівнюють нулю, означають, що команда не вдалася і нічого не буде робити. Позитивні значення означають, що команда була успішною та щось зробила. Brigadier забезпечує константу, яка вказує на успіх; `Command#SINGLE_SUCCESS`.
...
- `CommandDispatcher<CommandSourceStack> dispatcher` — використовується для реєстрації, аналізу та виконання команд. `S` — це тип джерела команди, який підтримує диспетчер команд.
...
Другий параметр визначає, чи транслювати зворотний зв'язок іншим модератори. Загалом, якщо команда має запитувати щось, фактично не впливаючи на світ, наприклад поточний час або рахунок якогось гравця, має бути `false`. Якщо команда щось виконує, наприклад змінює час або зміна чийогось рахунку, має бути `true`.
...
Brigadier [перенаправлятиме лише ноди команди з аргументами](https://github.com/Mojang/brigadier/issues/46). Якщо ви хочете перенаправити командний нод без аргументів, надайте конструктор `.executes()` із посиланням на ту саму логіку, що описана в прикладі.
...
Ви можете це зробити, але не рекомендується. Ви отримаєте `Commands` від сервера та додасте будь-які потрібні команди до його `CommandDispatcher`.
