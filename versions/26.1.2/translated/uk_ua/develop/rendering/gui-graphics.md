---
title: Малювання в GUI
description: Дізнайтеся, як використовувати клас GuiGraphicsExtractor для рендера різних форм, тексту та текстур.
authors:
  - IMB11
---
...
Клас `GuiGraphicsExtractor` є основним класом, який використовується для рендера в грі. Він використовується для рендера форм, тексту та текстур, і, як бачили раніше, використовується для маніпулювання `PoseStack` і використання `BufferBuilder`.

## Малювання фігур {#drawing-shapes}

Клас `GuiGraphicsExtractor` можна використовувати для легкого малювання **квадратних** фігур. Якщо ви хочете намалювати трикутники або будь-яку неквадратну форму, вам потрібно буде використовувати `BufferBuilder`.
...
Клас `GuiGraphicsExtractor` має вбудований менеджер ножиць. Це дає змогу легко прикріпити промальовування до певної області. Це корисно для рендера таких речей, як підказки або інші елементи, які не повинні рендеритися за межами певної області.
...
Загалом, рекомендується використовувати перевантаження, яке визначає параметри `textureWidth` і `textureHeight`. Це тому, що клас `GuiGraphicsExtractor` прийматиме ці значення, якщо ви їх не надасте, що іноді може бути неправильним.
...
### Малювання частини текстури {#drawing-a-portion-of-a-texture}
...
Клас `GuiGraphicsExtractor` має різні зрозумілі методи рендера тексту — задля стислості вони не розглядатимуться тут.

Скажімо, ми хочемо намалювати на екрані «Hello World». Для цього ми можемо використати метод `GuiGraphicsExtractor.text(…)`.
