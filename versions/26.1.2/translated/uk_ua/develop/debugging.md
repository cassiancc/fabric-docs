...
## Консоль та LOGGER {#console-and-logger}
...
- `Stack Trace` — якщо надано трасування стека винятку, журналювальник також може надрукувати це
...
Щоб ваш мод не засмічував файли журналів, виберіть відповідний рівень журналів для ваших журналів. Якщо дані, які ви реєструєте, актуальні лише під час розробки, настійно рекомендується використовувати рівень `debug`.

::: warning

Рівень `debug` усталено приховано, щоб показати його під час розробки, див. [журнал налагодження](#debug-logging).

:::

Якщо ви все ще не впевнені, який рівень журналу вибрати, хороше емпіричне правило полягає в тому, щоб використовувати лише неналагоджувальні рівні, якщо щось пішло не так. Розробників і користувачів пакетів модів не дуже хвилює, наприклад, ініціалізація предметів; вони хотіли б знати, якщо, наприклад, чому пакет даних не завантажується правильно.

### Журнал налагодження {#debug-logging}

Рівень журналу налагодження усталено приховано, але його можна спеціально ввімкнути в середовищі розробки за допомогою певного налаштування.

Для цього створіть файл налаштування `log4j-dev.xml` у кореневому каталозі вашого проєкту та додайте наступний вміст. Обов’язково замініть значення в атрибуті `name` на modid вашого мода. Це налаштування дозволяє показувати журнали налагодження з вашого мода на консолі.

@[code lang=xml](@/reference/26.1.2/log4j-dev.xml)

Потім у `build.gradle` скажіть Loom використовувати наше нове налаштування.

@[code lang=java transcludeWith=:::debug-logging](@/reference/26.1.2/build.gradle)

Тепер, коли ми надсилаємо повідомлення журналу налагодження, ми зможемо бачити їх у консолі.
...
![Консоль показує запис журналу налагодження](/assets/develop/debugging/debug_log.png)

::: tip

Якщо повідомлення про налагодження все ще показано, ваші налаштування запуску можуть бути застарілими. Щоб виправити це, повторно згенеруйте їх за допомогою завдання Gradle для вашої IDE.

:::

### Пошук проблем {#locating-issues}

Журналювальник друкує ID вашого мода перед лінією. Ви можете знайти його (<kbd>⌘/CTRL</kbd>+<kbd>F</kbd>), щоб виділити його.
...
Знову використаймо наш власний предмет як приклад. `CUSTOM_NAME` та `DataComponentType` предмета має змінитися, якщо він використовується по будь-якому камені.
...
| Вступ                    | Метод вступу для показу, що відбувається всередині. Якщо в одному рядку є кілька методів, ви можете вибрати, до якого переходити, натиснувши його. Це також необхідно для лямбда-виразів. |
...
Ви можете вносити обмежені зміни в код під час роботи екземпляра, використовуючи дію `Build > Build Project` зі значком молотка. Ви також можете розмістити значок поруч з елементом спадного меню `Run Configuration`, натиснувши ПКМ порожнє місце у верхній панелі меню IntelliJ.
...
У середовищі розробки ви можете знайти попередні журнали у вікні `Project` у теці `run > logs`, а звіти про збої — у теці `run > crash-reports`.
